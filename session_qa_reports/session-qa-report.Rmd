...
ss_checks <- check_session_ss(df)

assertthat::noNA(ss_checks)
```
...
```{r print-spreadsheet-checks2}
ss_checks_fmt[, c(1, 6:10)] %>%
  knitr::kable(., format = "html", escape = FALSE) %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

```{r print-spreadsheet-checks3}
...
```{r print-video-file-checks1}
check_videos_fmt[, 1:4] %>%
  knitr::kable(., format = "html", escape = FALSE) %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

```{r print-video-file-checks2}
